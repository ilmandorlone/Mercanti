...
dtype_status_player = np.dtype([
    ('tokens_violet', np.uint8),    # Gettoni viola del giocatore
    ('tokens_blue', np.uint8),      # Gettoni blu del giocatore
    ('tokens_green', np.uint8),     # Gettoni verdi del giocatore
    ('tokens_red', np.uint8),       # Gettoni rossi del giocatore
    ('tokens_black', np.uint8),     # Gettoni neri del giocatore
    ('tokens_gold', np.uint8),      # Gettoni oro del giocatore
    ('cards_violet', np.uint8),     # Carte viola del giocatore
    ('cards_blue', np.uint8),       # Carte blu del giocatore
    ('cards_green', np.uint8),      # Carte verdi del giocatore
    ('cards_red', np.uint8),        # Carte rosse del giocatore
    ('cards_black', np.uint8),      # Carte nere del giocatore
    ('points', np.uint8)            # Punti del giocatore
])

# Stato del gioco
dtype_status_game = np.dtype([
    ('round', np.uint8),                        # Round attuale
    ('player_id', np.uint8),                    # ID del giocatore attuale
    ('tokens_violet', np.uint8),                # Gettoni viola disponibili
    ('tokens_blue', np.uint8),                  # Gettoni blu disponibili
    ('tokens_green', np.uint8),                 # Gettoni verdi disponibili
    ...
    ('cards_level1', (np.uint8, 40)),           # Carte di livello 1
    ('cards_level2', (np.uint8, 30)),           # Carte di livello 2
    ('cards_level3', (np.uint8, 20)),           # Carte di livello 3
    ('nobles', (np.uint8, 10))                  # Nobili
])
