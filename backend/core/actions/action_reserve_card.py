...
class ActionReserveCard(Action):
    ...
    def execute_on_data_array(self, data: np.array):
        ...
        if self.context_match.tokens.gold > 0 and PlayerHelper.get_sum_tokens(player) < 10:
            data_after['players'][0]['tokens_' + ColorEnum.GOLD.value][player.id - 1] += 1
            # Rimuovi un gettone "gold" dal tavolo
            data_after['tokens_' + ColorEnum.GOLD.value] -= 1

        # Aggiorna lo stato della carta
        if card.level == 1:
            data_after['cards_level1'][0]['position'][card.id - 1] = int(StatusCardEnum_NP.RESERVED_PLAYER1) + player.id - 1
        elif card.level == 2:
            data_after['cards_level2'][0]['position'][card.id - 41] = int(StatusCardEnum_NP.RESERVED_PLAYER1) + player.id - 1
        else:
            data_after['cards_level3'][0]['position'][card.id - 71] = int(StatusCardEnum_NP.RESERVED_PLAYER1) + player.id - 1
